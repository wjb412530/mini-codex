...
三层防线：
1. 破坏性命令拦截（rm -rf /、mkfs、dd 等）
2. 子命令注入拦截（$(...)、反引号、zmodload）
3. 白名单沙盒判定（非白名单命令提示应放入沙盒）
"""
...
class BashTool(BaseTool):
    ...
    async def execute(self, command: str) -> str:
        ...
        # 安全检查 3：白名单沙盒判定
        needs_sandbox = should_use_sandbox(command)
        ...
        try:
            ...
            if needs_sandbox:
                output += "\n[安全提示] 该命令不在白名单内，生产环境应放入沙盒（Docker）隔离执行；当前未配置沙盒后端，已在宿主机直接执行。"
